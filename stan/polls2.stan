...
parameters {
    ...
    // reparametrization
    vector[D] mu_natl; // steps of random walk
    vector[N_state] mu_state[W]; // steps of randowm walk
...
model {
    ...
    val_poll = mu_natl[day] + 4*sd_firm*delta_firm[firm] + 4*bias_rv*type_rv
        + 4*bias_lv*type_lv + natl_error;
    for (i in 1:N) {
        if (!national[i]) {
    ...
    mu_natl[D] ~ normal(prior_natl_mean, prior_natl_sd);
    mu_state[W] ~ multi_normal_cholesky(prior_state_mean, chol_state_cov);
    for (d in 1:(D-1)) {
        mu_natl[d] ~ normal(mu_natl[d+1], sigma_natl);
    }
    for (w in 1:(W-1)) {
        mu_state[w] ~ normal(mu_state[w+1], sqrt(D_W)*sigma_state);
    }
