...
raw_tweets = read_rds("data/tweets.rdata") %>%
    ...
    mutate(tstamp = with_tz(mdy_hms(tstamp, tz="UTC"), "America/New_York"),
           ...
           date = force_tz(as_datetime(as_date(tstamp)+1),
...
tweets = full_join(tweets, missing, by=c("date", "hour")) %>%
    ...
    mutate(n_cuml = cumsum(n),
           n_rem = sum(n) - n_cuml) %>%
    ungroup %>%
    mutate(n_h1 = slide_vec(n, sum, .before=1),
...
m = lm(log(n_rem+1) ~ log(n_cuml+1)*hour + log(n_h1+1) + log(n_h6+1) +
...
d_test$y_pred = exp(predict(m, newdata=d_test)) - 1
y_act = d_test$n_rem
sd(y_pred - y_act)
cor(y_pred, y_act)^2
median(abs(y_pred - y_act))
...
d_test %>%
    ...
    group_by(day, hour) %>%
    summarize(se=sd(error), mad=mean(abs(error)), rsq=100*cor(n_cuml+n_rem, n_cuml+y_pred)^2) %>%
    pivot_longer(se:rsq) %>%
ggplot(aes(day+hour/24, value, color=name)) + geom_line()
